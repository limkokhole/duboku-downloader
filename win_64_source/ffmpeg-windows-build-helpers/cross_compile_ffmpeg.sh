...
build_ffmpeg() {
  ...
  cd $output_dir
    ...
    #A config_options+=" --enable-avresample" # guess this is some kind of libav specific thing (the FFmpeg fork) but L-Smash needs it so why not always build it :)

    #A for i in $CFLAGS; do
    #A  config_options+=" --extra-cflags=$i" # --extra-cflags may not be needed here, but adds it to the final console output which I like for debugging purposes
    #A done

    config_options+=" $postpend_configure_opts"

    #A if [[ "$non_free" = "y" ]]; then
    #A  config_options+=" --enable-nonfree --enable-decklink --enable-libfdk-aac"
    #A  # other possible options: --enable-openssl [unneeded since we use gnutls]
    #A fi
    ...
    #A if [[ "$do_debug_build" = "y" ]]; then
    #A  # not sure how many of these are actually needed/useful...possibly none LOL
    #A  config_options+=" --disable-optimizations --extra-cflags=-Og --extra-cflags=-fno-omit-frame-pointer --enable-debug=3 --extra-cflags=-fno-inline $postpend_configure_opts"
    #A  # this one kills gdb workability for static build? ai ai [?] XXXX
    #A  config_options+=" --disable-libgme"
    #A fi
    #A config_options+=" $extra_postpend_configure_options"

    echo hole options: "$config_options"
    do_configure "$config_options"
...
build_ffmpeg_dependencies() {
  ...
  #hole #build_fribidi # Uses dlfcn.
  #hole fribidi err #build_libass # Needs freetype >= 9.10.3 (see https://bugs.launchpad.net/ubuntu/+source/freetype1/+bug/78573 o_O) and fribidi >= 0.19.0. Uses fontconfig >= 2.10.92, iconv and dlfcn.
